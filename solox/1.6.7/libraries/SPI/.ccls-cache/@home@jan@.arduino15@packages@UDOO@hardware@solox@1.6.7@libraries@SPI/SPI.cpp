...
void SPIClass::setDataMode(uint8_t _mode) { mqx_spi_set_data_mode(_mode); }

void SPIClass::setBaud(uint32_t _baud) { mqx_spi_set_baud(_baud); }

/* Maximum bits of SPI burst. Because some flash commands must finish in one CS
   selection while ECSPI CS is controlled by SPI burst, thus we must try our
   best to put as much data as possible in one burst default = 4096*/
// I thought that was bit len
void SPIClass::setFrameSize(uint32_t _size) { mqx_spi_set_frame_size(_size); }
