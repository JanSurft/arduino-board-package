...
#if (MQX_USE_IDLE_TASK) && (MQX_ENABLE_LOW_POWER) &&                           \
    (MQX_IDLE_TASK_PARAMETER != 0)
...
#define MQX_NUM_TICK_FIELDS                                                    \
  ((MQX_MIN_BITS_IN_TICK_STRUCT + MQX_INT_SIZE_IN_BITS - 1) /                  \
   MQX_INT_SIZE_IN_BITS)
 ...
 */
/* int.c */
typedef void(_CODE_PTR_ INT_EXCEPTION_FPTR)(_mqx_uint, _mqx_uint, void *,
                                            void *);
...
typedef _mqx_uint(_CODE_PTR_ MQX_IPC_FPTR)(bool, _processor_number, _mqx_uint,
                                           _mqx_uint, _mqx_uint, ...);
...
typedef struct task_template_struct {
   ...
   *
   * The minimum value is 1, maximum is MAX_MQX_UINT. The field is ignored if
   * you call _task_create() or _task_create_blocked() or _task_create_at() with
   * a template index equal to 0 and a creation parameter set to a pointer to a
   * task template.
   */
  ...
  /*! \brief Pointer to a string name for tasks that MQX creates from the
   * template. */
  char *TASK_NAME;
   ...
   * \li MQX_AUTO_START_TASK (When MQX starts, it creates one instance of the
   * task.) \li MQX_FLOATING_POINT_TASK (Task uses the floating point
   * co-processor. MQX also saves floating-point registers as part of the task's
   * context.) \li MQX_TIME_SLICE_TASK (MQX uses round robin scheduling for the
...
typedef struct mqx_initialization_struct {
  ...
  /*! \brief Lowest address from which MQX allocates dynamic memory and task
   * stacks. */
  void *START_OF_KERNEL_MEMORY;
  /*!
   * \brief Highest address from which MQX allocates dynamic memory and task
   * stacks.
   *
...
extern INT_KERNEL_ISR_FPTR _int_install_kernel_isr(_mqx_uint,
                                                   INT_KERNEL_ISR_FPTR);
...
extern INT_EXCEPTION_FPTR _int_set_exception_handler(_mqx_uint,
                                                     INT_EXCEPTION_FPTR);
...
extern _mqx_uint _sched_set_rr_interval_ticks(_task_id, MQX_TICK_STRUCT_PTR,
                                              MQX_TICK_STRUCT_PTR);
...
extern _task_id _task_create_at(_processor_number, _mqx_uint, uint32_t, void *,
                                _mem_size);
...
extern TASK_EXCEPTION_FPTR _task_set_exception_handler(_task_id,
                                                       TASK_EXCEPTION_FPTR);
...
extern int32_t _time_diff_days(MQX_TICK_STRUCT_PTR, MQX_TICK_STRUCT_PTR,
                               bool *);
extern int32_t _time_diff_hours(MQX_TICK_STRUCT_PTR, MQX_TICK_STRUCT_PTR,
                                bool *);
extern int32_t _time_diff_minutes(MQX_TICK_STRUCT_PTR, MQX_TICK_STRUCT_PTR,
                                  bool *);
extern int32_t _time_diff_milliseconds(MQX_TICK_STRUCT_PTR, MQX_TICK_STRUCT_PTR,
                                       bool *);
extern int32_t _time_diff_nanoseconds(MQX_TICK_STRUCT_PTR, MQX_TICK_STRUCT_PTR,
                                      bool *);
extern int32_t _time_diff_picoseconds(MQX_TICK_STRUCT_PTR, MQX_TICK_STRUCT_PTR,
                                      bool *);
extern int32_t _time_diff_seconds(MQX_TICK_STRUCT_PTR, MQX_TICK_STRUCT_PTR,
                                  bool *);
extern int32_t _time_diff_microseconds(MQX_TICK_STRUCT_PTR, MQX_TICK_STRUCT_PTR,
                                       bool *);
extern _mqx_uint _time_diff_ticks(MQX_TICK_STRUCT_PTR, MQX_TICK_STRUCT_PTR,
                                  MQX_TICK_STRUCT_PTR);
extern int32_t _time_diff_ticks_int32(MQX_TICK_STRUCT_PTR, MQX_TICK_STRUCT_PTR,
                                      bool *);
...
#define HOST_TO_BE_UNALIGNED_SHORT(n, m)                                       \
  (((uint8_t *)(m))[0] = ((n) >> 8) & 0xFF, ((uint8_t *)(m))[1] = (n)&0xFF, (n))

#define HOST_TO_BE_UNALIGNED_LONG(n, m)                                        \
  (((uint8_t *)(m))[0] = ((n) >> 24) & 0xFF,                                   \
   ((uint8_t *)(m))[1] = ((n) >> 16) & 0xFF,                                   \
   ((uint8_t *)(m))[2] = ((n) >> 8) & 0xFF, ((uint8_t *)(m))[3] = (n)&0xFF,    \
   (n))

#define HOST_TO_LE_UNALIGNED_SHORT(n, m)                                       \
  (((uint8_t *)(m))[1] = ((n) >> 8) & 0xFF, ((uint8_t *)(m))[0] = (n)&0xFF, (n))

#define HOST_TO_LE_UNALIGNED_LONG(n, m)                                        \
  (((uint8_t *)(m))[3] = ((n) >> 24) & 0xFF,                                   \
   ((uint8_t *)(m))[2] = ((n) >> 16) & 0xFF,                                   \
   ((uint8_t *)(m))[1] = ((n) >> 8) & 0xFF, ((uint8_t *)(m))[0] = (n)&0xFF,    \
   (n))

#define SHORT_UNALIGNED_BE_TO_HOST(n)                                          \
  ((((uint16_t)(((uint8_t *)(n))[0])) << 8) | ((uint16_t)(((uint8_t *)(n))[1])))
...
#define SHORT_UNALIGNED_LE_TO_HOST(n)                                          \
  ((((uint16_t)(((uint8_t *)(n))[1])) << 8) | ((uint16_t)(((uint8_t *)(n))[0])))
...
/* Host to network / network to host conversion functions conforming to POSIX
 * API */
...
#define mqx_htonc(p, x) (((uint8_t *)(p))[0] = (x)&0xFF, (x))
...
#define mqx_htodc(p, x) (((uint8_t *)(p))[0] = (x)&0xFF, (x))
...
#define MSK10_SHARED_TRACE_FLAGS                                               \
  0x00000200 // mqx_mccuart_receive_task is running
