...
int UARTClass::available(void) {
  return (uint32_t)(SERIAL_BUFFER_SIZE + _rx_buffer->_iHead -
                    _rx_buffer->_iTail) %
         SERIAL_BUFFER_SIZE;
}
...
int UARTClass::read(void) {
  ...
  _rx_buffer->_iTail =
      (unsigned int)(_rx_buffer->_iTail + 1) % SERIAL_BUFFER_SIZE;
  return uc;
}
...
void UARTClass::IrqHandler(uint8_t rxChar) { _rx_buffer->store_char(rxChar); }
